...
object Spade
{
	...
	def get_new_list(posList1:List[Int],posList2:List[Int]):List[Int] ={
		...
		var min = poslist2.min
		var max = poslist1.max
	...
	def Spade(minSup:Int,TidList:collection.mutable.Map[String,collection.mutable.Map[Int,List[Int]]]):Unit ={
	...
	def main(args: Array[String]){
		...
		Spade(minSup,vertical_DB_1)
		get_maximal()

		Frequent_Sequences = Frequent_Sequences.sorted
		printf(" The Frequent Sequence are : \n")
		Frequent_Sequences.foreach { subSequence => printf("%s ",subSequence)}
		printf("\n The maximal ones are : \n")
		Maximal_Frequent_Sequences.foreach { subSequence => printf("%s ",subSequence)}
